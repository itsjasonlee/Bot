...
#include <string>
using namespace std;
//This program calculates if the amount of higher APM winners are greater than higher apm losers
int main()
{
	...
	int WinnerAPMOverLoserCount=0;
	int LoserAPMOverWinnerCount=0;
	int LoserAndWinnerEquals=0;
	...
	while(!ifile.eof())
	{
		...
		if(p1outcome == "Victory")
		{
			if(p1apm > p2apm)
			{
				WinnerAPMOverLoserCount+=1;
			}
			if(p2apm > p1apm)
			{
				LoserAPMOverWinnerCount+=1;
			}
			if(p2apm==p1apm)
			{
				LoserAndWinnerEquals+=1;
			}			
	...
	cout<< "Winners APM Over Losers APM: " << WinnerAPMOverLoserCount<<endl;
	cout<< "Losers APM Over Winners Count: " << LoserAPMOverWinnerCount<<endl;
	cout<< "Winners and Losers APM Equal: " << LoserAndWinnerEquals<<endl;
	cout<< "Total count: "<< (i-1)<<endl;
string loop;
	cin>> loop;
		return 1;
